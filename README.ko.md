...
> **기준을 세우고, 검증하며, 학습한다.**
>
> 개인용 AI 에이전트 거버넌스 플랫폼 — 중요한 것을 바꾸는 모든 Claude/LLM 세션을 감사·검증·학습한다.
...
**gijun-ai**는 사용자와 에이전트 사이에 위치하는 로컬 우선 거버넌스 레이어입니다:
...
| # | 계약 | 강제 방식 |
|---|------|----------|
| 1 | **Append-only 감사** — 기존 감사 행은 절대 수정되지 않고 새 행만 추가 | `audit_events`에 SQL `UPDATE`/`DELETE` 없음; `POST /audit/integrity-check`로 해시 체인 검증 |
| 2 | **전체 마이그레이션 체인 사전 검증** — 알려진 모든 마이그레이션이 순서대로 적용되기 전까지 서버는 요청을 받지 않음 | `server.ts`의 `app.listen()` 전 `assertSchemaChain([...])` 호출 |
| 3 | **콘텐츠 주소 기반 redaction** — redaction이 `payload`를 교체해도 `original_hash`가 유지되어 takedown 후에도 무결성 보존 | `audit_events.original_hash` 컬럼 + 변경 가능한 payload가 아닌 `original_hash`로 체인 연결 |
| 4 | **Fail-closed 인증** — `AGENTGUARD_TOKEN` 미설정 시 서버 시작 단계에서 종료; 모든 경로가 헤더 요구 | `server.ts`의 시작 가드 + `/health`를 제외한 모든 라우터의 `requireToken` 미들웨어 |
| 5 | **로컬 전용 바인딩** — 대기 주소는 `127.0.0.1`, 공개 인터페이스 불가 | `server.ts` 및 `transports.ts`의 하드코딩된 `HOST` |
| 6 | **비가역 작업 전 HITL 게이트** — 정책 또는 리스크로 게이트된 작업은 `done` 상태로 전이하기 전 명시적 인간 승인 이벤트 필요 | `hitl/gate.ts`의 `evaluateHitl()` + `POST /tasks/:id/hitl-approve` |
...
작업은 `complexity` 축(`trivial | standard | complex | critical`)을 가집니다. HITL 평가는 네 가지 차원을 결합합니다: **irreversibility**, **blast_radius**, **complexity**, **verify_fail**. 어느 한 축이라도 임계치를 넘으면 작업은 `hitl_wait` 상태로 전환되며, 오직 `POST /tasks/:id/hitl-approve`만이 이를 해제할 수 있습니다.
...
| 도구 | 대응 경로 | 목적 |
|------|----------|------|
| `list_tasks` | `GET /tasks` | 거버넌스 작업 목록. |
...
- **SHA-256 해시 체인** — `content_hash = sha256(original_hash + canonical(event))`; `chain_hash = sha256(prev_chain_hash + content_hash)`. 결정론적 canonical JSON으로 재현성 보장.
...
- **스키마 체인 사전 검증** — 서버는 `schema_migrations`가 예상된 순서(`001_initial → 002_original_hash → 003_original_hash_type → 004_cost_budget`)로 모든 마이그레이션을 나열하지 않으면 시작을 거부.
...
**대응**: standard 정책의 rate_limit 필드 + advisory 비용 예산. 폭주하는 토큰 비용에 의한 DoS는 `warning → critical → over_budget`에서 감지되어 감사 로그에 자동으로 표면화됨.
