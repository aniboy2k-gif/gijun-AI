...
| POST | `/tasks/external` | ✓ | 외부 시스템(예: bulletin-board CSR)에서 작업 upsert. `externalSource`+`externalId` 기준 멱등성. `{ id, created }` 반환. |
...
| GET | `/knowledge/drafts` | ✓ | HITL 검토 대기 중인 draft/candidate 목록. |
| POST | `/knowledge` | ✓ | 특정 계층에 지식 항목 생성. |
| POST | `/knowledge/search` | ✓ | FTS5 검색. Body: `{ query, limit?, project? }`. |
| POST | `/knowledge/da-candidate` | ✓ | DA 결과를 draft 지식 항목으로 미리 채움 (`status=draft`). |
| POST | `/knowledge/:id/promote` | ✓ | 레거시: `layer=candidate` 행을 `layer=incident`로 승격 (deprecated). |
| POST | `/knowledge/:id/nominate` | ✓ | draft → candidate 전이 (HITL 검토 시작). |
| POST | `/knowledge/:id/approve` | ✓ | candidate → approved 전이 (HITL 승인). |
| POST | `/knowledge/:id/revoke` | ✓ | approved → rejected 전이 (승인 취소). `reason` 필수. |
| POST | `/knowledge/:id/reject` | ✓ | draft\|candidate → rejected 전이. |
| POST | `/knowledge/:id/restore` | ✓ | rejected 항목을 새 draft 행으로 복원 (Option A: 새 행 + `supersedes_id`). |
...
**합계**: 42개 엔드포인트 (1개 health + 41개 인증 필요). v0.3.0–v0.3.1에서 추가: knowledge HITL 7개 + 외부 task sync 1개.

---

## MCP 도구 레퍼런스

REST 인터페이스에 1:1 매핑된 22개 도구. 명명 규칙: `get_/list_/search_/tail_/verify_/check_` = READ, 그 외는 WRITE. 모든 도구는 원자적 트랜잭션 내에서 서버 측에서 재검증됩니다 — `preflight_check`와 `check_budget` 결과는 advisory.
...
| `search_knowledge` | `POST /knowledge/search` | FTS5 검색 (approved 항목만). |
| `get_knowledge_drafts` | `GET /knowledge/drafts` | HITL 검토 대기 중인 draft/candidate 목록. |
...
| `create_knowledge` | `POST /knowledge` | 지식 항목 추가 (레거시). |
| `promote_knowledge` | `POST /knowledge/:id/promote` | 후보 → incident (레거시). |
| `report_incident` | `POST /incidents` | 인시던트 보고. |
| `create_da_candidate` | `POST /knowledge/da-candidate` | DA 결과로 draft 지식 항목 생성. |
| `nominate_knowledge_candidate` | `POST /knowledge/:id/nominate` | draft → candidate (HITL 검토 시작). |
| `approve_knowledge_candidate` | `POST /knowledge/:id/approve` | candidate → approved (최종 승인). |
| `revoke_knowledge_approval` | `POST /knowledge/:id/revoke` | approved → rejected (승인 취소). |
...
- **스키마 체인 사전 검증** — 서버는 `schema_migrations`가 예상된 순서(`001_initial → … → 005_policy_eval_index → 006_cost_parse_status → 007_knowledge_status → 008_external_sync`)로 모든 마이그레이션을 나열하지 않으면 시작을 거부.
...
### v0.3.1 (2026-04-29 출시)

bulletin-board → gijun-AI 단방향 outbox 연동 (P3).

- **`migrations/008_external_sync.sql`** — `external_id`, `external_source` 컬럼 + 부분 유니크 인덱스.
- **`POST /tasks/external`** — 멱등 upsert 엔드포인트; `{ id, created }` 반환.
- **`upsertExternalTask()`** — `task.external_sync` 감사 이벤트 포함 core 서비스 함수.
- **bulletin-board outbox** — `outbox_events` 테이블 + 폴링 워커 (`--once`/`--dry-run` 지원); `csr-log.js`의 `create-post`/`done`/`error` 커맨드에 silent-fail outbox 기록 훅 추가.
- 테스트 수: 133 (이전 126).

### v0.3.0 (2026-04-29 출시)

DA-chain 합의 기능 집합 (P0 + P1 + P2). 점진적 감사 검증, 다중 공급자 비용 파싱, HITL 게이트 지식 lifecycle 추가. MCP 도구: 22개 (이전 17개). 테스트 수: 126 (이전 78).

- **P0 — 감사 verify CLI** (`packages/core`): `VerifyOptions { fromId?, limit? }`, `VerifyResultItem` discriminated union, `--from`/`--limit`/`--json`/`--quiet` CLI 플래그, `chain_gap` 검출.
- **P1 — 다중 공급자 비용 파싱** (`packages/core`): `CostEntry` 타입, Strategy Pattern (`ICostParser`/`IPricingStrategy`), `ClaudeParser` + `OpenAIParser`, `cost_usd_micros INTEGER` canonical 컬럼, `parseAuto()` shape 기반 dispatch, migration 006.
- **P2 — Knowledge HITL lifecycle** (`packages/core`, `packages/server`, `packages/mcp-server`): `status ('draft'|'candidate'|'approved'|'rejected')`, `createDaCandidate()`, `nominateKnowledgeCandidate()`, `approveKnowledgeCandidate()`, `revokeKnowledgeApproval()` (상태 머신 닫힘), `rejectKnowledgeCandidate()`, `restoreFromRejected()` (Option A: 새 행 + `supersedes_id`), BEGIN IMMEDIATE + compare-and-swap, migration 007, REST 7개 + MCP 5개 추가.
...
- **`packages/web`** 읽기 전용 대시보드 (Vite + React + shadcn/ui) — **진행 중 (P4)**. Tasks · Audit · Cost · Knowledge 4탭. 기존 Express 서버의 `GET /`에서 서빙.
